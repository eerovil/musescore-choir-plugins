        ...
        for voice in measure.findall('.//voice'):
            ...
            for chord in voice.findall('.//Chord'):
                ...
                if stem_voice != voice_index:
                    ...
                    logging.debug(f"Reversed voice found in staff {staff.get('id')}, measure {index}, voice {voice_index}, direction {stem_direction}")
                    logging.debug(f"Chord: {etree.tostring(chord, pretty_print=True).decode('utf-8')}")
...
def handle_staff(staff, direction):
    ...
    for measure in staff.findall('.//Measure'):
        index += 1
        reversed_voices = REVERSED_VOICES_BY_STAFF_MEASURE.get(staff_id, {}).get(index, False)
        if reversed_voices:
        ...
        for voice in measure.findall('.//voice'):
            ...
            if voice_index == voice_to_remove:
                ...
                logging.debug(f"Removed voice {voice_index} from staff {staff_id}, measure {index}")
            else:
                logging.debug(f"Keeping voice {voice_index} in staff {staff_id}, measure {index}")
    ...
    for chord in staff.findall('.//Chord'):
        ...
        if stem_direction is not None:
            ...
            logging.debug(f"Set StemDirection to {stem_direction.text} for chord in staff {staff_id}")
...
def main(input_path, output_path):
    ...
    output_content = etree.tostring(root, pretty_print=True, encoding='UTF-8').decode('UTF-8')
